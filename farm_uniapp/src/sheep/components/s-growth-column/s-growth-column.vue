...
<template>
  <view class="ss-goods-wrap">
    ...
    >
      ...
      <image class="lg-img-box" :src="isArray(state.seedList) ? state.seedList[0].files[0].url : state.seedList[0].files" mode="aspectFill"></image>
      <view class="lg-goods-content ss-flex-1 ss-flex-col ss-row-between ss-p-b-10 ss-p-t-20">
        <view>
          ...
          >
            地块:{{ growth.farmlandLesseeVO.landArea.landAreaName }}
          </view>
          <view
              v-if="goodsFields.subtitle?.show"
              class="lg-goods-subtitle ss-m-t-10 ss-line-1"
          ...
          <view
              ...
              :style="[{ color: subTitleColor, background: subTitleBackground }]"
          >
            预期产量:{{ growth.yieldExpectation }}公斤/亩
          </view>
          <!--服务类型，租赁周期-->
      ...
      <slot name="cart">
        <view class="seed-box ss-flex ss-col-center ss-row-center"
              v-if="props.growth.growthStage === 10 "     >
          播种期
        </view>
        <view class="growing-box ss-flex ss-col-center ss-row-center"
              v-if="props.growth.growthStage === 20 "     >
          生长期
        </view>
        <view class="harvest-box ss-flex ss-col-center ss-row-center"
              v-if="props.growth.growthStage === 30 "    >
          收获期
        </view>
        <view class="finish-box ss-flex ss-col-center ss-row-center"
              v-if="props.growth.growthStage === 40 "     >
          结束期
        </view>
      </slot>
    </view>
...
<script setup>
  ...
  state.seedList = props.farmlandLesseeVO.seedList;
  ...
  if(props.farmlandLesseeVO.growthStage !== 0){
    state.buttonShow = false;
  }
  function continuePay(){
    sheep.$router.redirect('/pages/pay/index', {
      orderId: props.farmlandLessee.farmlandLesseeOrder.orderId,
      totalAmount: props.farmlandLessee.lesseeAmount,
      farmlandLesseeId:props.farmlandLessee.farmlandLesseeId,
