...
<template>
  <view class="ss-goods-wrap">
    ...
    >
      ...
      <image class="xs-img-box" :src="isArray(data.files) ? data.files[0].url : data.files" mode="aspectFit"></image>
      <view
      ...
      >
        ...
        >
          {{ data.farmlandName }}33111
        </view>
        <view
          v-if="goodsFields.lesseeAmount?.show"
          class="xs-goods-price font-OPPOSANS"
          :style="[{ color: goodsFields.price.color }]"
        >
          <text class="price-unit ss-font-24">{{ priceUnit }}</text>
          {{ isArray(data.lesseeAmount) ? data.lesseeAmount[0] : data.lesseeAmount }}
        </view>
      </view>
    </view>
    ...
    <view v-if="size === 'md'" class="md-goods-card ss-flex-col" :style="[elStyles]" @tap="onClick">
      ...
      <image
        class="md-img-box"
        :src="  isArray(data.files) ? data.files[0].url : data.files "
        mode="widthFix"
      ...
      >
        ...
        <view class="ss-flex ss-col-bottom">
          ...
          >
            <text class="price-unit ss-font-24">{{ priceUnit }}</text>
            {{ isArray(data.leasePrice) ? data.leasePrice[0] : data.leasePrice }}
          </view>
