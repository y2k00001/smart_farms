    ...
    <view v-if="state.iconStatus && state.pagination.total > 0" class="goods-list ss-m-t-20">
      ...
      >
        <s-seeds-column
          ...
          :topRadius="10"
          :bottomRadius="10"
          @click="sheep.$router.go('/pages/seeds/index', { id: item.id })"
        ></s-seeds-column>
      </view>
    </view>
    <view
...
<script setup>
  ...
  async function getList(Sort, Order, seedIds, keyword, page = 1, list_rows = 6) {
    state.loadStatus = 'loading';
    const res = await sheep.$api.seeds.list({
      sort: Sort,
      order: Order,
      seedIds: !keyword ? seedIds : '',
      list_rows,
      keyword: keyword,
      page,
    });
    if (res.code === 200) {
      let couponList = _.concat(state.pagination.data, res.rows);
      state.pagination = {
        ...res.rows,
        data: couponList,
        total: res.total
  ...
  onLoad((options) => {
    state.seedIds = options.seedIds;
    getList(state.currentSort, state.currentOrder, state.seedIds, state.keyword);
  });
  // 上拉加载更多
  onReachBottom(() => {
    loadmore();
