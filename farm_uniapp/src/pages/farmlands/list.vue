...
<!--          <text v-if="state.iconStatus" class="sicon-goods-list">31</text>-->
<!--          <text v-else class="sicon-goods-card">31</text>-->
<!--        </view>-->
<!--      </view>-->
<!--    </su-sticky>-->
        ...
        >
          <template v-slot:cart>
            <button class="ss-reset-button cart-btn" :style="[buyStyle]">
              {{  state.buyNowStyle.mode === 1 ?  state.buyNowStyle.text : '' }}
            </button>
          </template>
        </s-farmlands-column>
      </view>
...
<script setup>
  ...
  const buyStyle = computed(() => {
    if ( state.buyNowStyle.mode == 1) {
      // button
      return {
        background: `linear-gradient(to right, ${ state.buyNowStyle.color1}, ${ state.buyNowStyle.color2})`,
      };
    }

    if ( state.buyNowStyle.mode == 2) {
      // image
      return {
        width: '54rpx',
        height: '54rpx',
        background: `url(${sheep.$url.cdn( state.buyNowStyle.src)}) no-repeat`,
  ...
  async function getList() {
    ...
    const res = await sheep.$api.farmlands.list(params, { page:state.pagination.current_page - 1, size: state.pagination.size });
